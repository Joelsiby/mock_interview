...
const InterviewSection = () => {
  ...
  const handleSurpriseMe = () => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
    }, 2000); // This mimics an asynchronous operation
  };
  ...
  return (
    <div className="min-h-screen bg-blue-200 flex flex-col items-center justify-center py-12">
      ...
      <div className="grid grid-cols-2 gap-10">
          ...
          <div className="flex justify-center mb-4">
            {Object.keys(topicsData).map((category) => (
              <button
                ...
                className={`text-white bg-gradient-to-r from-sky-300 to-sky-800 hover:from-sky-400 hover:to-sky-900 font-semibold py-2 px-4 rounded-lg shadow-md m-2 ${activeCategory === category ? 'ring-4 ring-sky-300' : ''}`}
              >
          ...
          <ul className="space-y-4">
            {topicsData[activeCategory].map((topic, index) => (
              <li key={index}
                ...
                className={`bg-gray-100 p-2 rounded-lg shadow-xl cursor-pointer ${selectedTopic === topic ? 'bg-sky-300' : ''}`}
              >
                {topic}
              </li>
            ))}
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg flex flex-col justify-between" style={{ width: '400px', height: '400px' }}>
          {displayQuestion && (
          ...
          <div className="flex justify-between mb-2">
            ...
            <button
              ...
              className="text-white bg-purple-500 flex items-center justify-center flex-1 ml-1 py-2 px-4 rounded shadow"
            >
              {loading ? (
                <>
          ...
          <button
            ...
            className={`text-white bg-green-600 w-full py-2 px-4 rounded shadow ${!displayQuestion ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            Start practicing
          </button>
        </div>
